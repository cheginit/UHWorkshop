...
#====================================================;
vSpace = FiniteElement("RT", mesh.ufl_cell(), 1)
pSpace = FiniteElement("DG", mesh.ufl_cell(), 0)
TH = vSpace * pSpace
...
class BoundarySource(Expression):
    def __init__(self, mesh,degree): # init method is like constructor in c, mesh is the arg class accepts
        self.mesh = mesh
    def eval_cell(self, values, x, ufc_cell): # later our G.eval_cell has a meaning
...
#-------------------------------------------------;
bc_Bottom = DirichletBC(wSpace.sub(0),G,bottom)
bc_Top_Rigid = DirichletBC(wSpace.sub(0),G,topRigid)
...
L = dot(rhob,w)*dx - dot(w,n) * p_Production * ds(5)\
   - dot(w,n) * p_Injection * ds(2)\
   - dot(w,n) * p_Injection * ds(1)
...
#=======================================;
file = File('Velocity_RT0.pvd')
file << v

file = File('Pressure_RT0.pvd')
file << p
